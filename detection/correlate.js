...
async function main() {
  ...
  const [openAnomalies, attachedAnomalyIds] = await Promise.all([
  ...
  console.log(`Processed ${unattachedAnomalies.length} open anomalies; created ${correlatedIncidents} correlated incidents and ${singleSourceIncidents} single-source incidents.`);
}
